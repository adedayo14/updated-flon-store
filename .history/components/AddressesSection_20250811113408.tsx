...
const AddressCard: React.FC<AddressProps> = ({ address }) => {
  ...
  const { addresses, setAddresses, accountId } = useAddressContext();
      ...
      >
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            {/* Remove mobile checkmark - no longer needed */}
            <div className={`text-lg font-semibold ${
              address.default ? 'text-teal-600' : 'text-teal-600'
            }`}>
              {address.name}
            </div>
          </div>
          
          {/* Default badge - top right with proper light green background */}
          {address.default && (
            <span 
              className="inline-flex items-center rounded-full px-3 py-1.5 text-sm font-medium shadow-sm border bg-green-100 text-teal-600 border-green-200"
        ...
        <div className="space-y-1 mb-6">
          <div className="text-base text-teal-600">{address.address1}</div>
          {address.address2 && (
            <div className="text-base text-teal-600">{address.address2}</div>
          )}
          <div className="text-base text-teal-600">
            {address.city}, {address.state} {address.zip}
          </div>
          <div className="text-base text-teal-600">{address.country}</div>
          {address.phone && (
            <div className="text-base text-teal-600">Phone: {address.phone}</div>
          )}
          {address.company && (
            <div className="text-base text-teal-600">Company: {address.company}</div>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          ...
          {!address.default && (
            <button
              ...
              className="text-teal-600 hover:text-teal-700 text-sm font-medium underline decoration-2 underline-offset-4 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 rounded transition-colors duration-200"
              aria-pressed="false"
          ...
          {/* Edit and Delete buttons - with special handling for default addresses */}
          <div className="flex gap-2 sm:ml-auto">
            <Button
              ...
              small
              className="whitespace-nowrap focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 border-teal-600 text-teal-600 hover:bg-teal-50"
            >
              Edit
            </Button>
            {address.default ? (
              <Button
                ...
                buttonStyle={BUTTON_STYLE.SECONDARY}
                small
                className="whitespace-nowrap border-gray-200 text-gray-400 cursor-not-allowed bg-gray-50"
                disabled
              >
                Delete
              </Button>
              ...
              <Button
                ...
                buttonStyle={BUTTON_STYLE.SECONDARY}
                small
                className="whitespace-nowrap focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 border-teal-600 text-teal-600 hover:bg-teal-50 hover:text-teal-700 hover:border-teal-700"
              >
                Delete
              </Button>
            )}
...
const AddressesSection: React.FC = () => {
  const { addresses, accountId } = useAddressContext();
