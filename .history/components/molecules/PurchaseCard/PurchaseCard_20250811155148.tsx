...
const PurchaseCard: React.FC<PurchaseCardProps> = ({
  ...
  return (
    <div className="border-outline rounded-xl border bg-background-primary p-6 relative">
      {/* Fixed-width container for status and images - prevents shifting */}
      <div className="absolute top-4 right-4 z-10 w-64 flex flex-col items-end">
        <StatusIndicator status={status} type={props.type} />
        <div className="mt-4 grid grid-cols-3 gap-2 justify-end">
          {productsImages.map((image, i) => (
            <div
              key={image.alt + i}
              className="relative aspect-square w-16 h-16 md:w-20 md:h-20">
              <Image
                src={image.src}
                layout="fill"
                alt={image.alt}
      ...
      <div className="md:flex md:justify-between">
        <div className="md:flex-1 pr-72"> {/* Increased padding to account for fixed-width container */}
          {/* Header with title only - status moved to absolute position */}
          <div className="mb-4">
