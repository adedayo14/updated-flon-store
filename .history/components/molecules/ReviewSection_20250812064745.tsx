...
const ReviewSection: React.FC<ReviewSectionProps> = ({ productId }) => {
  ...
  const submitReview = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setShowForm(false);
      // No immediate add to list because it’s pending; just show toast-like message
        ...
        {eligibility ? (
          eligibility.loggedIn ? (
            eligibility.hasPurchased ? (
              <div className="mb-6">
                {!showForm ? (
                  <button className="bg-primary text-white px-4 py-2 rounded" onClick={() => setShowForm(true)}>
                    Write a review
                  </button>
                ) : (
              ...
              <p className="text-sm text-gray-600 mb-6">Only customers who purchased this item can write a review.</p>
            )
          ) : (
            <p className="text-sm text-gray-600 mb-6">Please sign in to write a review.</p>
          )
        ) : null}
