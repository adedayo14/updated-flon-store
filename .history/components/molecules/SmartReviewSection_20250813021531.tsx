...
const CustomDropdown: React.FC<CustomDropdownProps> = ({ options, value, onChange, placeholder, className }) => {
  ...
  return (
    <div className={`relative ${className}`}>
      <button
        type="button"
        className="w-full px-4 py-2.5 border border-gray-200 rounded-full bg-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 text-left flex items-center justify-between whitespace-nowrap"
        onClick={() => setIsOpen(!isOpen)}
      >
        <span className="text-gray-700 text-sm">
          {selectedOption ? 
            (selectedOption.label.includes('★') && selectedOption.value !== '' ? 
              selectedOption.label.split('').map((char, i) => (
                <span key={i} className={char === '★' ? 'star-teal' : 'text-gray-300'}>
                  {char === '☆' ? '★' : char}
                </span>
              )) : 
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
          {options.map((option, index) => (
              ...
              >
                <span className={option.label.includes('★') && option.value !== '' ? 'star-display' : ''}>
                  {option.label.includes('★') && option.value !== '' ? 
                    option.label.split('').map((char, i) => (
                      <span key={i} className={char === '★' ? 'star-teal' : 'text-gray-300'}>
                        {char === '☆' ? '★' : char}
                      </span>
                    )) : 
