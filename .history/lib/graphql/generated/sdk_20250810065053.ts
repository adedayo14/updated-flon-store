...
export const getSdk = (client: any) => ({
  ...
  getCategoryPaths: () => Promise.resolve({ data: { categories: { results: [] } } }),
  getMenus: () => Promise.resolve({ data: { contentMenus: { results: [] } } }),
  ...
  getPageBySlug: ({ slug }: { slug: string }) => Promise.resolve({ data: { contentPageBySlug: null } }),
});
