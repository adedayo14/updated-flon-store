...
export const getSampleReviews = (): Review[] => {
  const sampleReviews: Review[] = [
    ...
    // Pending reviews for admin approval
    {
      ...
      rating: 5,
      title: 'Love this sustainable floss!',
      review_body: 'Been using this for 3 months now and absolutely love it. The silk is so gentle on my gums and the refill system is brilliant. Highly recommend!',
    ...
    },
    {
      ...
      review_body: 'The floss quality is good and I like the eco-friendly aspect, but it\'s quite pricey compared to regular floss. Worth it if you can afford the premium.',
      status: 'pending' as const,
      is_verified_purchase: true,
    ...
    },
    {
      ...
      review_body: 'I don\'t see what all the fuss is about. It\'s just floss at the end of the day, but costs 3x more than regular floss. The container is nice though.',
      status: 'pending' as const,
...
export const getProductReviews = async (productId: string): Promise<Review[]> => {
  ...
  // Filter reviews by product ID (case-insensitive) and only approved ones for public display
...
// Get all reviews (for admin purposes) - includes pending and rejected
...
export const getPendingReviews = async (): Promise<Review[]> => {
  const allReviews = getSampleReviews();
  return allReviews.filter(review => review.status === 'pending');
};
