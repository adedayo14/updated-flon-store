...
export const getProductListingData = async (
  ...
  try {
    ...
    // But DON'T fetch all products - that's what's causing the 15-20 minute delay
    const [categoriesResult, settingsData] = await Promise.all([
      getCategories(),
      // Get store settings for layout configuration
      rest.storefront.get<{ store?: any }>('/settings').catch(() => ({ data: {} }))
    ]);

    const { categories, settings: defaultSettings } = categoriesResult;
    const storeSettings = settingsData?.data?.store || {};

    // Create enhanced settings from store configuration
    const settings: ProductsLayoutSettings = {
      ...defaultSettings,
      showProductsPrice: storeSettings.products?.show_price !== false,
      showProductsDescription: storeSettings.products?.show_description !== false,
      showFeaturedCategories: storeSettings.products?.show_featured_categories === true,
      productsPerRow: storeSettings.products?.per_row || defaultSettings.productsPerRow || 4,
      enableQuickAdd: storeSettings.products?.enable_quick_add !== false,
      ...(storeSettings.products?.featured_categories && {
        featuredCategories: storeSettings.products.featured_categories
      })
    };

    // Create minimal attribute filters from categories instead of all products
    // This avoids the expensive getAllProducts query
    const attributeFilters: ProductFilterOption[] = categories
    ...
    try {
      ...
      throw new Error(`Unable to fetch product listing data: ${error.message}`);
    }
  }
};
