...
export async function getProductBySlug(
  ...
  try {
    ...
    const productPromise = Promise.race([
      client.getProduct({
        slug,
        currency,
        locale,
    ...
    // If no product found, return minimal fallback
    if (!product) {
      console.warn(`Product not found for slug: ${slug}, returning fallback`);
...
export const getStoreSettings = async (locale?: string) => {
  try {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => 
      setTimeout(() => reject(new Error('Store settings timeout')), 10000)
    );
    ...
    const [storeData, menusData] = await Promise.race([
      settingsPromise,
      timeoutPromise
    ]);
    ...
    console.warn('Store settings API failed, using fallback:', error?.message || 'Unknown error');
    trackAPIFailure('getStoreSettings');

    // Return fallback settings to prevent build failure
    return {
      name: 'Flon',
      homePage: 'home',
      currency: {
