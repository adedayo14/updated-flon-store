import { graphql, rest } from 'lib/swell';
...
export const fetchProductsPaginated = async (options: ProductsQueryOptions = {}): Promise<ProductsQueryResult> => {
  ...
  try {
    ...
    const response = await rest.storefront.get(`/products?${params.toString()}`);
    ...
    const graphqlResponse = await graphql.request(graphqlQuery, {
      limit,
      page,
      sort,
...
export const fetchProductsSample = async (limit: number = 8): Promise<PurchasableProductData[]> => {
  try {
