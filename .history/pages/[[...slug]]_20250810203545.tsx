...
export const getStaticPaths: GetStaticPaths = async ({
  ...
  try {
    // Add timeout protection for build-time API calls
    const timeoutPromise = new Promise<never>((_, reject) => 
      setTimeout(() => reject(new Error('getStaticPaths timeout')), 8000)
    );

    const client = getGQLClient();
    const storeSettingsPromise = client.getStoreSettings();
    
    const { data: { storeSettings } } = await Promise.race([
    ...
    const contentPagesPromise = client.getContentPages();
    const { data: { contentPages } } = await Promise.race([
      contentPagesPromise,
      timeoutPromise
    ]);
    ...
    console.warn('Error in getStaticPaths, using fallback:', error?.message || 'Unknown error');
...
const propsCallback: GetStaticProps<StaticPageProps> = async (context) => {
  try {
    // Add timeout protection for build-time API calls
    const timeoutPromise = new Promise<never>((_, reject) => 
      setTimeout(() => reject(new Error('getStaticProps timeout')), 8000)
    );
    ...
    const fetchPagePromise = fetchPageData(slug as string, locale);
    ...
    console.warn('Error in getStaticProps, returning 404:', error?.message || 'Unknown error');
    return {
      notFound: true,
    };
  }
