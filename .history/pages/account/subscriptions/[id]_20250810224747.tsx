    ...
    <div className="max-w-4xl mx-auto p-8">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-8 mb-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
          ...
          {subscription?.orderScheduleText && (
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <svg className="w-4 h-4 opacity-70" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="space-y-4">
          {subscription.orderItems.map((item, index) => (
            <div key={index} className="flex gap-4 items-center py-5 border-b border-gray-100 last:border-b-0">
              <div className="w-16 h-16 bg-gradient-to-br from-gray-100 to-gray-200 border border-gray-200 rounded-lg
                flex items-center justify-center text-2xl">
                🪒
              </div>
              <div className="flex-1">
                <div className="text-base font-medium text-black">{item.product?.name}</div>
                <div className="text-sm text-gray-600">
                  {item.product?.description || `Quantity: ${item.quantity}`}
                </div>
              </div>
              <div className="text-right">
                <div className="text-base font-semibold text-black">{formatPriceByCurrency(subscription?.currency)(item.price ?? 0)}</div>
      ...
      <div className="mt-10">
        ...
        <div className="bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-200 rounded-xl p-10 text-center">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            ...
            <button
              onClick={() => setEditSubscriptionOpen(true)}
              className="flex flex-col items-center gap-2 p-6 border rounded-lg bg-white
                transition-all duration-200 transform hover:-translate-y-0.5
                border-gray-900 text-gray-900 hover:bg-gray-900 hover:text-white"
            >
              <div className="text-sm font-semibold">{text.edit.label}</div>
              <div className="text-xs opacity-75 text-center leading-tight">
            ...
            {status !== SUBSCRIPTION_STATUS.PAUSED ? (
              ...
              >
                <div className="text-sm font-semibold">Pause subscription</div>
                <div className="text-xs opacity-75 text-center leading-tight">
                  Temporarily stop deliveries and billing
                </div>
              </button>
            ) : (
              <button
                ...
                className="flex flex-col items-center gap-2 p-6 border rounded-lg bg-white
                  ...
                  border-amber-600 text-amber-600 hover:bg-amber-600 hover:text-white"
              >
                <div className="text-sm font-semibold">Resume subscription</div>
                <div className="text-xs opacity-75 text-center leading-tight">
                  Restart deliveries and billing
                </div>
              </button>
            ...
            {status !== SUBSCRIPTION_STATUS.CANCELED && (
              ...
              >
                <div className="text-sm font-semibold">Cancel subscription</div>
                <div className="text-xs opacity-75 text-center leading-tight">
                  End this subscription permanently
                </div>
              </button>
            ...
            {showInvoiceBtn && (
              ...
              >
                <div className="text-sm font-semibold">Add Invoice</div>
                <div className="text-xs opacity-75 text-center leading-tight">
                  Add additional items to billing
                </div>
              </button>
            )}
          </div>
