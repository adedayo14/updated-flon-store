    ...
    <div className="max-w-4xl mx-auto p-8">
      ...
      <div className="mb-12">
        <GhostButton
          ...
          className="inline-flex items-center gap-2 text-teal-600 hover:text-teal-800 text-sm mb-6 transition-colors">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-8 mb-8">
        ...
        <div className="flex justify-between items-start mb-6 gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-teal-600 mb-1 tracking-tight">
              {subscription.name}
            </h1>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
          {headerLeftColumn.map(([label, value]) => (
            <div key={label} className="flex flex-col">
              <div className="text-xs text-teal-600 uppercase tracking-wider mb-1">{label}</div>
              <div className="text-sm font-medium text-teal-600">{value}</div>
            </div>
          ))}
          <div className="flex flex-col">
            <div className="text-xs text-teal-600 uppercase tracking-wider mb-1">Total</div>
            <div className="text-sm font-medium text-teal-600">{subscription.grandTotal}</div>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-200 rounded-xl p-10 mb-8">
        <h2 className="text-xl font-semibold text-teal-600 mb-2 tracking-tight text-left">Manage subscription</h2>
        <p className="text-sm text-teal-600 mb-8 text-left">You&apos;re in control.</p>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {/* Edit Button */}
          <button
            onClick={() => setEditSubscriptionOpen(true)}
            className="flex flex-col items-center gap-2 p-6 border border-teal-600 text-teal-600 bg-white rounded-lg
              transition-all duration-200 transform hover:-translate-y-0.5 hover:bg-teal-600 hover:text-white"
          >
      ...
      {/* Product Card */}
      <div className="bg-white border border-gray-200 rounded-xl p-8 mb-8">
        <h2 className="text-lg font-semibold text-teal-600 mb-4 tracking-tight">Your subscription</h2>
          ...
          {subscription.orderItems.map((item, index) => (
            <div key={index} className="flex gap-4 items-center py-5 border-b border-gray-100 last:border-b-0">
              ...
              <div className="flex-1">
                <div className="text-base font-medium text-teal-600">{item.title}</div>
                <div className="text-sm text-teal-600">
                  {item.options?.join(', ') || `Quantity: ${item.quantity}`}
                </div>
              </div>
              <div className="text-right">
                <div className="text-base font-semibold text-teal-600">{item.price}</div>
                <div className="text-xs text-teal-600">{subscription?.billingScheduleText}</div>
              </div>
            </div>
          ))}
        </div>
