    ...
    <div className="max-w-4xl mx-auto p-8">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-8 mb-8">
        ...
        <div className="flex justify-between items-start mb-6 gap-4">
          <div>
            <h1 className="text-2xl font-semibold mb-1 tracking-tight" style={{ color: '#008080' }}>
              {subscription.name}
            </h1>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
          {headerLeftColumn.map(([label, value]) => (
            <div key={label} className="flex flex-col">
              <div className="text-xs uppercase tracking-wider mb-1" style={{ color: '#008080' }}>{label}</div>
              <div className="text-sm font-medium" style={{ color: '#008080' }}>{value}</div>
            </div>
          ))}
          <div className="flex flex-col">
            <div className="text-xs uppercase tracking-wider mb-1" style={{ color: '#008080' }}>Total</div>
            <div className="text-sm font-medium" style={{ color: '#008080' }}>{subscription.grandTotal}</div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
          {subscription?.billingScheduleText && (
            <div className="flex items-center gap-2 text-sm" style={{ color: '#008080' }}>
      ...
      <div className="bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-200 rounded-xl p-10 mb-8">
        <h2 className="text-xl font-semibold mb-2 tracking-tight text-left" style={{ color: '#008080' }}>Manage subscription</h2>
        <p className="text-sm mb-8 text-left" style={{ color: '#008080' }}>You&apos;re in control.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <button
            ...
            className="flex flex-col items-center gap-2 p-6 border bg-white rounded-lg
              transition-all duration-200 transform hover:-translate-y-0.5 hover:text-white"
            style={{ borderColor: '#008080', color: '#008080', '--tw-hover-bg-color': '#008080' } as React.CSSProperties}
      ...
      {/* Product Card */}
      <div className="bg-white border border-gray-200 rounded-xl p-8 mb-8">
        <h2 className="text-lg font-semibold mb-4 tracking-tight" style={{ color: '#008080' }}>Your subscription</h2>
          ...
          {subscription.orderItems.map((item, index) => (
            <div key={index} className="flex gap-4 items-center py-5 border-b border-gray-100 last:border-b-0">
              ...
              <div className="flex-1">
                <div className="text-base font-medium" style={{ color: '#008080' }}>{item.title}</div>
                <div className="text-sm" style={{ color: '#008080' }}>
                  {item.options?.join(', ') || `Quantity: ${item.quantity}`}
                </div>
              </div>
              <div className="text-right">
                <div className="text-base font-semibold" style={{ color: '#008080' }}>{item.price}</div>
                <div className="text-xs" style={{ color: '#008080' }}>{subscription?.billingScheduleText}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-8">
        <h2 className="text-lg font-semibold mb-4 tracking-tight" style={{ color: '#008080' }}>Invoice breakdown</h2>
        <div className="space-y-3">
          {subscription.summaryRows.map((row, index) => (
            <div key={index} className="flex justify-between items-center py-3 border-b border-gray-100">
              <div className="text-sm" style={{ color: '#008080' }}>{row.label}</div>
              <div className="text-sm font-medium" style={{ color: '#008080' }}>{row.value}</div>
            </div>
          ))}
          <div className="flex justify-between items-center pt-4 border-t border-gray-200">
            <div className="text-sm font-semibold" style={{ color: '#008080' }}>{subscription.totalRow.label}</div>
            <div className="text-sm font-semibold" style={{ color: '#008080' }}>{subscription.totalRow.value}</div>
          </div>
        </div>
      </div>
