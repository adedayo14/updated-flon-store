...
const AdminModerationPage: NextPage = () => {
  ...
  const loadReviews = async (which: 'pending' | 'all' = 'pending') => {
    setLoading(true);
    setError(null);
    try {
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-2">
                      {item.review.status === 'pending' ? (
                        <>
                          <button onClick={() => moderate(item.review.id, 'approved')} className="bg-green-600 text-white px-3 py-1 rounded disabled:opacity-50" disabled={loading}>
                            Approve
                          </button>
                          <button onClick={() => moderate(item.review.id, 'rejected')} className="bg-red-600 text-white px-3 py-1 rounded disabled:opacity-50" disabled={loading}>
                            Reject
                          </button>
                        </>
                      ) : (
  ...
  }, [isAuthed, error, filter, reviews, loading]);
