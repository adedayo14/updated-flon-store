...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    // Fetch products from Swell API using public key
    let products: Product[] = [];
    
    try {
      console.log('🔍 Fetching products from Swell API...');
      
      const response = await fetch('https://raw-foods.swell.store/api/products', {
        headers: {
      ...
      if (response.ok) {
        ...
        console.log('✅ Successfully fetched', data.results?.length || 0, 'products from Swell API');
        ...
        console.error('❌ Failed to fetch products from Swell API:', response.status, response.statusText);
