                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-500">
                        <div className="mb-1">
                          <strong>Product:</strong> {productNames[review.product_id] || 'Unknown Product'}
                        </div>
                        <div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    // Get product names directly from Swell API (same approach as products page)
    ...
    try {
      ...
      for (const productId of uniqueProductIds) {
        ...
        try {
          const response = await fetch(`https://raw-foods.swell.store/api/products/${productIdStr}`, {
            headers: {
              'Authorization': `Basic ${Buffer.from('raw-foods:sk_E0kCv4HK7wyp0ooMIWE0qQd9YjsJKsJI').toString('base64')}`
            }
          });
          
          if (response.ok) {
            const product = await response.json();
            productNames[productIdStr] = product.name || 'Unknown Product';
            console.log('Fetched product name:', productIdStr, '->', product.name);
          } else {
            console.log('Failed to fetch product:', productIdStr, response.status);
            productNames[productIdStr] = 'Unknown Product';
          }
        } catch (error) {
          console.error('Error fetching product', productIdStr, error);
          productNames[productIdStr] = 'Unknown Product';
        }
      }
    } catch (error) {
