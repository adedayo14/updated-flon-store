                  ...
                  <div className="flex items-start justify-between">
                    ...
                    {review.status === 'pending' && (
                      <div className="flex space-x-2 ml-6">
                        <button
                          className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded shadow-md disabled:opacity-50 disabled:cursor-not-allowed border-0"
                          onClick={() => handleModerate(review.id, 'approve')}
                          disabled={loading[review.id]}
                          style={{ background: loading[review.id] ? '#9CA3AF' : '#059669' }}
                        >
                          {loading[review.id] ? 'Approving...' : 'Approve'}
                        </button>
                        <button
                          className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded shadow-md disabled:opacity-50 disabled:cursor-not-allowed border-0"
                          onClick={() => handleModerate(review.id, 'reject')}
                          disabled={loading[review.id]}
                          style={{ background: loading[review.id] ? '#9CA3AF' : '#DC2626' }}
                        >
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    // Get product names using direct REST API calls (more reliable for SSR)
    const productNames: Record<string, string> = {};
    
    // Map of known product IDs to names (you can extend this based on your products)
        ...
        if (knownProducts[productIdStr]) {
          ...
          try {
            ...
            if (response.ok) {
              ...
              productNames[productIdStr] = product.name || `Product ${productIdStr.slice(-8)}`;
            } else {
              console.log(`❌ Failed to fetch product ${productIdStr}: Status ${response.status}`);
              productNames[productIdStr] = `Product ${productIdStr.slice(-8)}`;
            }
          } catch (apiError) {
            console.log(`❌ API error for product ${productIdStr}:`, apiError);
            productNames[productIdStr] = `Product ${productIdStr.slice(-8)}`;
          }
        }
      }
