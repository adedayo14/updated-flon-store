...
import type { AdminReviewListItem } from 'types/shared/reviews';

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<{ reviews: AdminReviewListItem[]; total: number } | string>,
) {
  if (req.method !== 'GET') {
  ...
  try {
    // Get pending reviews using our local service
    const pendingReviews = getPendingReviews();

    // Transform to admin format
    const adminReviews: AdminReviewListItem[] = pendingReviews.map((review) => ({
      review: {
        id: review.id,
