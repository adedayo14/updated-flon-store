...
export function formatStoreSettings(
  ...
  // Add safety checks for settings structure
  const storeName = settings?.store?.name || 'Demo Store';
  const settingsValues = settings?.values || { header: {}, footer: {} };
...
export const getCurrencyFromSettings = (settings: any) => {
  // Add safety check for settings structure
