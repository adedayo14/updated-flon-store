...
export function createReview(reviewData: Omit<Review, 'id' | 'created_at' | 'updated_at' | 'helpful_count'>): Review {
  ...
  const currentNextId = Number.isFinite(reviewsFileData.nextId) ? reviewsFileData.nextId : 1;
  const newId = currentNextId > 0 ? currentNextId : 1;
  ...
  reviewsFileData.nextId = newId + 1;
